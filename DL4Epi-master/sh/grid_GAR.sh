...
do
    ...
    do
	    ...
	    option="--normalize ${NORM} --epochs 1500 --save_dir save --save_name gar.${LOG}.w-${window}.h-${HORIZON}.pt --data ${DATA} --model GAR --horizon ${HORIZON} --window ${window} --metric 0 --weight_decay ${decay}"
            cmd="stdbuf -o L python ./main.py ${option} | tee log/gar/gar.${LOG}.d-${decay}.w-${window}.h-${HORIZON}.out"
	    echo $cmd
            eval $cmd
        done
