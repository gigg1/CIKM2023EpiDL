...
do
    ...
    do
        ...
        do
            ...
            option="--normalize ${NORM} --epochs 3000 --save_dir save --save_name ar_reg.${LOG}.w-${window}.h-${HORIZON}.pt --data ${DATA} --model AR --horizon ${HORIZON} --window ${window} --metric 0 --weight_decay ${decay}"
            cmd="stdbuf -o L python ./main.py ${option} | tee log/ar/ar.${LOG}.d-${decay}.w-${window}.h-${HORIZON}.out"
            echo $cmd
            eval $cmd
        done
