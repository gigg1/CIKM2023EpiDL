...
class cola_gnn_epi(nn.Module):
    def __init__(self, args, data): 
        ...
        # self.EpiLinear1 = nn.Linear(self.w, 20)
        # self.EpiLinear2 = nn.Linear(20, )

        # self.Beta=nn.Parameter(torch.ones([self.m,],dtype=torch.float32)*0.5)
        # self.Gamma=nn.Parameter(torch.ones([self.m,],dtype=torch.float32)*0.2)
        # self.NextGenerationMatrix = torch.Tensor(self.m, self.m)
    ...
    def forward(self, x, feat=None):
        ...
        # batch number / location number / location number
        # ---- sparse
        IfAdjacent = self.adj[:,:]
        ...
        # given the fixed epidemiological parameters
        # Beta = (torch.ones([self.m,])*0.5).repeat(b, 1).view(b, self.m)
        # Gamma = (torch.ones([self.m,])*0.2).repeat(b, 1).view(b, self.m)
        ...
        # for batch in range(0,b):
        #     W = torch.Tensor(self.m, self.m)
        #     adj_diagValue = torch.Tensor(self.m, self.m)
        #     for i in range(0,self.m):
        #         WeightedSum=0
        #         for j in range(0,self.m):
        #             if i != j :
        #                 WeightedSum = WeightedSum + Adj_Epi[batch][i][j]
        #                 # W[i][j] = 0
        #                 # adj_diagValue[i][j] = 0
        #         W[i][i] = WeightedSum
        #         adj_diagValue[i][i] = Adj_Epi[batch][i][i]

        #     Gamma = torch.diag(self.Gamma)
        #     Beta = torch.diag(self.Beta)
        #     # ******
        #     tmp1 = Gamma-((Adj_Epi[batch].T - adj_diagValue) - W)
        #     # ******
        #     NextGenerationMatrix[batch] = Beta.matmul(tmp1.inverse())
        ...
        #             tmp1[i][i] = 1
            ...
            # print (Beta.shape)
            # print (tmp1)
            # print (NextGenerationMatrix[batch].shape)
            # print (NextGenerationMatrix[batch])

        # self.NextGenerationMatrix = NextGenerationMatrix

        # print ("")
        # print (Gammas)
        # print (Betas)
        # print ("self.NextGenerationMatrix")
        # print (self.NextGenerationMatrix)
        # print (self.NextGenerationMatrix.shape)
        # print ("self.Beta")
        # print (self.Beta)
        # print ("self.Gamma")
        # print (self.Gamma)
